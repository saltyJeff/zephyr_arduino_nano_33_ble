...
static inline int arduino_gpio_pinMode(struct arduino_gpio_t *gpios, int pin, int mode)
{
	return gpio_pin_configure(gpios->gpios[pin / 32], pin % 32, mode);
}
static inline int arduino_gpio_digitalWrite(struct arduino_gpio_t *gpios, int pin, int value)
{
	return gpio_pin_set(gpios->gpios[pin / 32], pin % 32, value);
}
static inline int arduino_gpio_digitalRead(struct arduino_gpio_t *gpios, int pin)
{
