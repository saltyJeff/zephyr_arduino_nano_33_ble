...
/* note: IDK how but all the nordic headers got automagically imported. Cool. */
static int board_internal_sensors_init(const struct device *dev)
{
	...
	/* Arduino uses software to disable RTC1, but I disabled it using DeviceTree */
	...
	/* the PCB designers decided to use GPIO's as power pins for the internal sensors */
	...
	/* add a busy wait because the internal sensors need time to boot up */
	k_busy_wait(500 * 1000);
	return 0;
}
SYS_INIT(board_internal_sensors_init, PRE_KERNEL_1, 0);
